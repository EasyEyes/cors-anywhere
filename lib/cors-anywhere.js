...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    try {
    // Handle useTypekitToken header
    if (req.headers['usetypekittoken'] === 'true') {
      var typekitToken = process.env.typeKitToken;
      if (typekitToken) {
        req.headers['x-typekit-token'] = typekitToken;
      }
    }
      // Remove the useTypekitToken header as it's only for proxy control
      delete req.headers['usetypekittoken'];
    } catch (error) {
      console.error('Error handling useTypekitToken header:', error);
    }
